...
const responseSchema = z.object({
  sanctionsPotentialMatch: z.boolean(),
  pepOrIpPotentialMatch: z.boolean(),
  adverseMediaPotentialMatch: z.boolean(),
  screeningReference: z.string().min(1).max(200),
  screenedAt: z.string().datetime(),
}).strict();

export class HttpComplianceScreeningProvider implements IComplianceScreeningProvider {
  readonly name = "HTTP_COMPLIANCE_SCREENING";
  ...
  async screen(input: ComplianceScreeningInput): Promise<ComplianceScreeningResult> {
    ...
    try {
      ...
      if (!parsed.success) throw new Error("Compliance-screening provider returned an invalid response.");
      return parsed.data;
    } finally {
...
export default HttpComplianceScreeningProvider;
