...
export function decideEKYC(input: DecisionInput, config: EKYCConfig): DecisionResult {
  ...
  if (!input.identity.nidMatched) return { ...common, status: "PENDING_MANUAL_REVIEW", reasons: ["NID_MISMATCH"] };
  if (!input.identity.dateOfBirthMatched) return { ...common, status: "PENDING_MANUAL_REVIEW", reasons: ["DOB_MISMATCH"] };
  ...
  if (!input.liveness.passed || input.liveness.attackSignals.length) {
    return { ...common, status: "PENDING_MANUAL_REVIEW", reasons: ["LIVENESS_FAILED"] };
  }

  const quality = input.identity.faceQuality;
  if (!quality.faceDetected) return { ...common, status: "PENDING_MANUAL_REVIEW", reasons: ["FACE_NOT_DETECTED"] };
  if (!quality.singleFaceDetected) return { ...common, status: "PENDING_MANUAL_REVIEW", reasons: ["MULTIPLE_FACES"] };
  ...
  if (input.fingerprint) {
    ...
    if (!input.fingerprint.matched || input.fingerprint.score < config.thresholds.fingerprintMatch) {
      return { ...common, status: "PENDING_MANUAL_REVIEW", reasons: ["FINGERPRINT_MISMATCH"] };
    }
  }
  if (input.identity.faceMatchScore < config.thresholds.faceManualReview) {
    return { ...common, status: "PENDING_MANUAL_REVIEW", reasons: ["FACE_SCORE_REJECTED"] };
  }
  ...
  return { ...common, status: "PENDING_MANUAL_REVIEW", reasons: ["AUTOMATED_CHECKS_COMPLETED"] };
}
