...
const livenessSchema = z
  .object({
    ...
    attackSignals: z.array(
      z.enum(["SCREEN_REPLAY", "PRINT_ATTACK", "MASK", "MULTIPLE_FACES"])
    ),
  })
  .strict();
...
/**
 * Production adapter for an EC/Porichoy contract.
 *
 * The endpoint response bodies must be mapped by your approved gateway/proxy
 * to the canonical schemas above. Do not infer production URLs, field names,
 * or credentials from public examples.
 */
  ...
  private async post(path: string, payload: unknown): Promise<unknown> {
    ...
    try {
      ...
      if (response.status >= 500 || response.status === 429) {
        throw new ECProviderError(
          "EC gateway is temporarily unavailable.",
          "UNAVAILABLE",
          true
        );
      }
      ...
      if (!response.ok) {
        throw new ECProviderError(
          "EC gateway rejected the request.",
          "REQUEST_REJECTED",
          false
        );
      }
...
export { ECProviderError } from "./ECProviderError.js";
