...
transporter
  ...
  .then(() => {
    console.log(
      "SMTP READY: transporter verified successfully."
    );
  })
  .catch((error) => {
    console.error(
      "SMTP VERIFY FAILED — emails will NOT be delivered until this is fixed:",
      {
        message:
          error?.message,
        code: error?.code,
    ...
    try {
      const info =
        await transporter.sendMail({
          ...
          text:
            `Your Coffer verification code is ${otp}. It expires in 10 minutes.`,
...
Secure payments • Smarter financial control
      ...
      if (
        ...
        throw new Error(
          `SMTP server rejected the recipient address: ${info.rejected.join(", ")}`
        );
      }
    } catch (error: any) {
      console.error(
...
export const sendMerchantInvoiceEmail =
  async ({
    email,
    customerName,
    merchantName,
    ...
    const info =
      await transporter.sendMail({
        ...
        text: [
          `Hello ${customerName},`,
          "",
          `${merchantName} sent you invoice ${invoiceNumber}.`,
          `Amount: ${currency} ${amount}`,
          `Due date: ${formattedDueDate}`,
          "",
          `View invoice: ${invoiceUrl}`,
          "",
          "This invoice is securely delivered through Coffer Digital Wallet.",
        ].join("\n"),
...
">
  <div style="padding:40px 16px;">
    ...
    ">
      ...
      ">
        <div style="
          ...
          padding:13px 16px;
          border-radius:15px;
          background:linear-gradient(135deg,#9b5cff,#6627db);
          font-size:19px;
          font-weight:800;
        ">
          C
        ...
        <p style="
          ...
          line-height:1.7;
        ">
          Securely delivered through Coffer Digital Wallet.
        </p>
      </div>
      ...
      <div style="padding:36px;">
        <p style="
          ...
          line-height:1.8;
        ">
          Hello ${safeCustomer}, ${safeMerchant} has sent you
          a new invoice.
        </p>
        ...
        ">
          ...
          <p style="
            ...
            line-height:1.7;
          ">
            Do not forward this email or share its invoice link
    ...
    if (
      ...
      throw new Error(
        `SMTP rejected the invoice recipient: ${info.rejected.join(
          ", "
        )}`
      );
    }
