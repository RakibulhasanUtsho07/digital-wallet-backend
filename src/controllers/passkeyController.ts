...
type PaymentAuthenticationOperation =
  | "TRANSFER"
  | "MERCHANT_PAYMENT";
...
function config(): {
  ...
  if (
    ...
    throw new Error(
      "WEBAUTHN_RP_ID must match WEBAUTHN_ORIGIN."
    );
  }
...
function operationOf(
  value: unknown
): PaymentAuthenticationOperation | null {
  /*
   * Missing operation remains TRANSFER for backward
   * compatibility with the existing Send Money page.
   */
  if (
    value === undefined ||
    value === null ||
...
export async function createPasskeyRegistrationOptions(
  ...
  if (!user) {
    res.status(403).json({
      ...
      message:
        "Verified KYC is required to register a passkey.",
    });
...
export async function verifyPasskeyRegistration(
  ...
  /*
   * findOneAndDelete makes registration flow single-use.
   */
  const flow =
  ...
  if (!flow?.challenge) {
    res.status(400).json({
      ...
      message:
        "Passkey registration expired. Please try again.",
    });
  ...
  /*
   * A credential must never belong to two users.
   */
  const existingCredential =
    await PasskeyCredential.findOne({
      credentialId:
        credential.id,
    })
      .select(
  ...
  if (
    existingCredential &&
    existingCredential.userId.toString() !==
      userId
  ) {
    res.status(409).json({
      success: false,
...
/* =========================================================
   ...
   POST /api/passkeys/payment/options
...
export async function createPaymentAuthenticationOptions(
  ...
  if (!operation) {
    res.status(400).json({
      ...
      message:
        "Invalid payment authentication operation.",
    });
  ...
  if (
    ...
    if (
      ...
      res.status(400).json({
        ...
        message:
          "This payment does not support Coffer passkey authorization.",
      });
    ...
    /*
     * A preassigned payment must only be authorized by
     * that customer.
     */
    if (
      payment.customerId &&
    ...
    /*
     * All financial values come from MongoDB.
     * The frontend cannot change the authorized amount
     * or merchant.
     */
    operationHash =
      merchantPaymentOperationHash({
      ...
      res.status(400).json({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Invalid transfer authorization request.",
      });
  ...
  if (
    ...
    res.status(404).json({
      ...
      message:
        "Register a device passkey first.",
    });
...
export async function verifyPaymentAuthentication(
  ...
  /*
   * Authentication challenge becomes unusable after
   * this request.
   */
  const flow =
    await WebAuthnFlow.findOneAndDelete({
  ...
  if (
    ...
    res.status(400).json({
      ...
      message:
        "Payment authentication expired. Please try again.",
    });
  ...
  if (!passkey) {
    res.status(400).json({
      ...
      message:
        "The selected passkey is not registered.",
    });
  ...
  /*
   * Counter update helps detect cloned/replayed
   * authenticators.
   */
  passkey.counter =
    verification
  ...
  const authorization =
    await issuePaymentAuthorization(
      userId,
      flow.operationHash
    );
