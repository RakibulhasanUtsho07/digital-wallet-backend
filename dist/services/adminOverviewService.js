...
exports.clearAdminOverviewCache = clearAdminOverviewCache;
exports.recordOverviewExport = recordOverviewExport;
exports.overviewToCsv = overviewToCsv;
...
async function getAdminOverview(range) {
    ...
    const end = new Date();
    const start = startOfUtcDay(new Date(end.getTime() -
        (days - 1) *
            DAY_MS));
    ...
    ======================================================= */
    const [totalUsers, previousUsers, activeWallets, previousActiveWallets, revenueTotals, revenueSeries, pendingKyc, previousPendingKyc, riskAlerts, previousRiskAlerts, openSupport, failedTransactions, healthRows,] = await Promise.all([
        /* Users */
        ...
        /* Revenue */
        periodMoneyTotals(revenueEvents, start, end, previousStart, [
            "completed",
            "success",
            "posted",
            "SUCCESS",
    ...
    ======================================================= */
    const transactionRows = await transactions
        .find({
        createdAt: {
            $gte: previousStart,
            $lte: end,
    ...
    for (const row of transactionRows) {
        const senderId = getObjectIdString(row.senderId);
        const receiverId = getObjectIdString(row.receiverId);
        if (senderId) {
    ...
    for (const user of userRows) {
        userNameMap.set(String(user._id), stringValue(user.name, "User"));
    }
    /* =======================================================
    ...
    for (const row of transactionRows) {
        const decrypted = decryptTransaction(row, userNameMap);
        if (decrypted) {
    ...
    ======================================================= */
    let currentTransactionVolume = 0;
    let previousTransactionVolume = 0;
    ...
    ======================================================= */
    const transactionSeries = buildTransactionSeries(start, days, decryptedTransactions);
    /* =======================================================
       STATUS BREAKDOWN
    ======================================================= */
    const statusRows = buildStatusRows(decryptedTransactions, start, end);
    /* =======================================================
       RECENT TRANSACTIONS
    ======================================================= */
    const recentTransactions = decryptedTransactions
        ...
        .slice(0, 8)
        .map(toOverviewTransaction);
    /* =======================================================
       MERGE CHART DATA
    ======================================================= */
    const series = mergeDailySeries(start, days, transactionSeries, revenueSeries);
    /* =======================================================
       RESPONSE
    ======================================================= */
    const response = {
        ...
        kpis: {
            ...
            transactionVolume: metric(currentTransactionVolume, previousTransactionVolume),
        ...
        series,
        transactionStatuses: statusRows,
        recentTransactions,
...
/* =========================================================
   RECORD EXPORT
========================================================= */
async function recordOverviewExport(actorId, range) {
    const db = mongoose_1.default.connection.db;
    if (!actorId ||
        !db) {
        return;
    }
    const actor = mongoose_1.Types.ObjectId.isValid(actorId)
        ? new mongoose_1.Types.ObjectId(actorId)
        : actorId;
    await db
...
/* =========================================================
   DECRYPT TRANSACTION
========================================================= */
function decryptTransaction(row, userNameMap) {
    const amount = decryptAmount(row.amountEncrypted);
    if (amount === null) {
        /*
    ...
    const senderName = senderId
        ? userNameMap.get(senderId) ??
            "Sender"
        : "Sender";
    const receiverName = receiverId
        ? userNameMap.get(receiverId) ??
            "Receiver"
        : "Receiver";
    const reference = decryptOptionalValue(row.referenceEncrypted);
    return {
        ...
        receiverName,
        amount,
        currency: stringValue(row.currency, "BDT").toUpperCase(),
...
/* =========================================================
   DECRYPT AMOUNT
========================================================= */
function decryptAmount(value) {
    if (!isEncryptedData(value)) {
        return null;
    }
    try {
        const decrypted = (0, crypto_js_1.decryptData)(value);
        ...
        console.error("ADMIN OVERVIEW TRANSACTION AMOUNT DECRYPT ERROR:", error instanceof Error
            ? error.message
            : error);
        return null;
    }
...
========================================================= */
function decryptOptionalValue(value) {
    if (!isEncryptedData(value)) {
        return undefined;
    }
    try {
        ...
        console.error("ADMIN OVERVIEW OPTIONAL DATA DECRYPT ERROR:", error instanceof Error
            ? error.message
            : error);
        return undefined;
    }
...
function isEncryptedData(value) {
    ...
    const record = value;
    return (typeof record.encrypted ===
        "string" &&
        typeof record.iv ===
            "string" &&
...
/* =========================================================
   TRANSACTION SERIES
========================================================= */
function buildTransactionSeries(start, days, transactions) {
    const map = new Map();
...
========================================================= */
async function dailyMoneySeries(collection, start, end, includeCount) {
    ...
    for (const row of rows) {
        ...
        const date = created
            .toISOString()
            .slice(0, 10);
        const amount = resolveMoneyMajor(row);
        const current = map.get(date) ?? {
            amount: 0,
            count: 0,
        };
...
========================================================= */
async function periodMoneyTotals(collection, start, end, previousStart, completedStatuses) {
    const rows = await collection
        .find({
        createdAt: {
            $gte: previousStart,
...
/* =========================================================
   RESOLVE PLAIN MONEY
========================================================= */
function resolveMoneyMajor(row) {
    if (row.amountMinor !==
        undefined &&
        row.amountMinor !==
            null) {
        return (numberValue(row.amountMinor) / 100);
    }
    if (row.revenueMinor !==
        undefined &&
        row.revenueMinor !==
            null) {
        return (numberValue(row.revenueMinor) / 100);
    }
    if (row.feeMinor !==
        undefined &&
        row.feeMinor !==
            null) {
        return (numberValue(row.feeMinor) / 100);
    }
    /*
...
/* =========================================================
   MERGE CHART SERIES
========================================================= */
function mergeDailySeries(start, days, transactionRows, revenueRows) {
    const transactionMap = new Map(transactionRows.map((row) => [
        String(row._id),
        row,
    ]));
    const revenueMap = new Map(revenueRows.map((row) => [
        String(row._id),
        row,
    ]));
    const result = [];
    for (let index = 0; index < days; index += 1) {
        ...
        result.push({
            date,
            volume: round(numberValue(transaction?.amountMajor)),
...
/* =========================================================
   STATUS ROWS
========================================================= */
function buildStatusRows(transactions, start, end) {
    const counts = {
        completed: 0,
        pending: 0,
        failed: 0,
...
========================================================= */
function toOverviewTransaction(transaction) {
    return {
        id: transaction.id,
...
/* =========================================================
   SERVICE HEALTH
========================================================= */
function buildServiceHealth(rows) {
    const groups = new Map();
    for (const row of rows) {
        const name = stringValue(row.service ??
            row.source ??
            row.module, "API");
        ...
        current.total +=
            1;
        const severity = String(row.level ??
            row.severity ??
            "").toLowerCase();
        if ([
            "error",
        ...
        return {
            ...
            status: errorRate >=
                ...
                : errorRate >=
                    ...
                    ? "degraded"
                    : "operational",
            uptimePercent: round((1 -
                errorRate) *
                100),
            latencyMs: value.total
                ? Math.round(value.latency /
                    value.total)
                : 0,
        };
    });
...
function minorToMajor(value) {
    return round(numberValue(value) /
        100);
}
/* =========================================================
   NUMBER
