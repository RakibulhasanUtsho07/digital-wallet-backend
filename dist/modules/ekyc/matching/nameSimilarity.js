...
/* =========================================================
   JARO SIMILARITY
========================================================= */
function jaroSimilarity(leftValue, rightValue) {
    ...
    for (let leftIndex = 0; leftIndex < left.length; leftIndex += 1) {
        const start = Math.max(0, leftIndex -
            matchDistance);
        const end = Math.min(leftIndex +
            matchDistance +
            1, right.length);
    ...
    for (let index = 0; index < left.length; index += 1) {
        if (leftMatches[index]) {
            matchedLeft.push(left[index]);
        }
    }
    for (let index = 0; index < right.length; index += 1) {
        if (rightMatches[index]) {
            matchedRight.push(right[index]);
        }
    }
    let transpositions = 0;
    for (let index = 0; index <
        matchedLeft.length; index += 1) {
...
/* =========================================================
   JARO-WINKLER
========================================================= */
function calculateJaroWinkler(left, right) {
    const baseScore = jaroSimilarity(left, right);
    const leftCharacters = Array.from(left);
    const rightCharacters = Array.from(right);
    let commonPrefixLength = 0;
    while (commonPrefixLength <
        Math.min(4, leftCharacters.length, rightCharacters.length) &&
        leftCharacters[commonPrefixLength] ===
            rightCharacters[commonPrefixLength]) {
...
/* =========================================================
   PUBLIC MATCHING
========================================================= */
function jaroWinkler(leftInput, rightInput) {
    const left = normalizeName(leftInput);
    const right = normalizeName(rightInput);
    const leftScript = detectScript(left);
    const rightScript = detectScript(right);
    const crossScript = (leftScript ===
        "BENGALI" &&
        rightScript ===
            "LATIN") ||
        (leftScript ===
    ...
    const directScore = calculateJaroWinkler(left, right);
    /*
     * Token-sorted comparison handles minor name-order
     * differences without automatically changing scripts.
     */
    const sortedScore = calculateJaroWinkler(sortNameTokens(left), sortNameTokens(right));
    const finalScore = Math.max(directScore, sortedScore);
    return {
        score: Math.round(finalScore * 10_000) / 100,
        requiresTransliteration: false,
        normalizedLeft: left,
        normalizedRight: right,
    };
}
function bestNameScore(claimedName, candidates) {
    const results = candidates
        .filter((candidate) => Boolean(candidate?.trim()))
        .map((candidate) => jaroWinkler(claimedName, candidate));
    return (results.sort((left, right) => right.score -
        left.score)[0] || {
        score: 0,
        requiresTransliteration: false,
