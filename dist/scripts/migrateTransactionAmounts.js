...
async function migrateTransactionAmounts() {
    try {
        ...
        for (const transaction of transactions) {
            try {
                ...
                if (!Number.isFinite(amount) ||
                    amount <= 0) {
                    console.log(`⚠️ Skipped ${transaction._id}: invalid amount`);
                    skipped++;
                    continue;
                }
                /* =================================================
                ...
                if (!Number.isSafeInteger(minorUnits) ||
                    minorUnits <= 0) {
                    console.log(`⚠️ Skipped ${transaction._id}: invalid minor-unit amount`);
                    skipped++;
                    continue;
                }
                /* =================================================
                ...
                ================================================== */
                await Transaction_js_1.Transaction.collection.updateOne({
                    _id: transaction._id,
                    amountEncrypted: {
                        $exists: false,
                    },
                ...
                failed++;
                console.error(`❌ Failed transaction ${transaction._id}:`, error instanceof Error
                    ? error.message
                    : error);
            }
        }
        ...
        console.log("==================================================");
        if (failed === 0) {
            console.log("✅ Transaction amount migration completed.");
        }
    }
    catch (error) {
        console.error("MIGRATION ERROR:", error instanceof Error
            ? error.message
            : error);
        process.exitCode = 1;
    }
